...
import com.wy.springcloud.dao.OrderDao;
...
public class OrderServiceImpl implements OrderService {
    ...
    /**
     * 模拟创建订单全调用链
     * 创建订单->调用库存服务扣减库存->调用账户服务扣减账户余额->修改订单状态（简述：下订单->扣库存->减余额->改状态）
     *
     * @param order 订单
     */
    @Override
    //seata的全局异常注解，name自定义，要保证唯一
    @GlobalTransactional(name = "wyuan-create-order", rollbackFor = Exception.class)
    public void create(Order order) {
        log.info("----->开始新建订单，模拟微服务调用过程");
        //1 新建订单
